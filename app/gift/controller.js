...
async function getGift(start, end) {
  var query = `SELECT 
    ...
    LEFT JOIN (SELECT COUNT(gift_id) total, room_id FROM gift GROUP BY room_id) g
      on g.room_id = r.room_id
    WHERE start_live_date between '${start} 00:00:01' AND '${end} 23:59:59'`;
  var data = await dbMysql.sequelize.query(query, { type: QueryTypes.SELECT });
  return data;
}
