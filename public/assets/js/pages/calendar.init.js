...
function upcomingEvent(e) {
    e.sort(function (e, t) {
    ...
    }), document.getElementById("upcoming-event-list").innerHTML = null, e.forEach(function (e) {
        var t = e.title,
            n = e.end;
        n = "Invalid Date" == n || null == n ? null : (l = new Date(n).toLocaleDateString(), new Date(l).toLocaleDateString("en-GB", {
            day: "numeric",
            month: "short",
            year: "numeric"
        }).split(" ").join(" ")), str_dt(e.start) === str_dt(e.end) && (n = null);
        var a = e.start;
        a = "Invalid Date" == a || null == a ? null : (l = new Date(a).toLocaleDateString(), new Date(l).toLocaleDateString("en-GB", {
            day: "numeric",
            month: "short",
            year: "numeric"
        }).split(" ").join(" "));
        ...
        e = e ? " to " + e : "";
        u_event = "<div class='card mb-3'>                        <div class='card-body'>                            <div class='d-flex mb-3'>                                <div class='flex-grow-1'><i class='mdi mdi-checkbox-blank-circle me-2 text-" + i[2] + "'></i><span class='fw-medium'>" + a + d + "</span></div>                                <div class='flex-shrink-0'><small class='badge badge-soft-primary ms-auto'>" + n + e + "</small></div>                            </div>                            <h6 class='card-title fs-16'> " + t + "</h6>                            <p class='text-muted text-truncate-two-lines mb-0'> " + l + "</p>                        </div>                    </div>", document.getElementById("upcoming-event-list").innerHTML += u_event
    })
}
...
document.addEventListener("DOMContentLoaded", function () {
    flatPickrInit();
    var v = new bootstrap.Modal(document.getElementById("event-modal"), {
        keyboard: !1
    });
    ...
    function o() {
        document.getElementById("form-event").reset(), document.getElementById("btn-delete-event").setAttribute("hidden", !0), v.show(), i.classList.remove("was-validated"), i.reset(), g = null, d.innerText = "Add Event", document.getElementById("edit-event-btn").setAttribute("data-id", "new-event"), document.getElementById("edit-event-btn").click(), document.getElementById("edit-event-btn").setAttribute("hidden", !0)
    }
    ...
    var c = !0,
        ...
        E = new FullCalendar.Calendar(e, {
            ...
            eventClick: function (e) {
                ...
                a == e ? (document.getElementById("event-time").setAttribute("hidden", !0), flatpickr(document.getElementById("timepicker1"), {
                    enableTime: !0,
                    noCalendar: !0,
                    dateFormat: "H:i"
                    ...
                    dateFormat: "H:i",
                    defaultDate: e
                }), document.getElementById("event-timepicker1-tag").innerHTML = tConvert(a), document.getElementById("event-timepicker2-tag").innerHTML = tConvert(e)), d.innerText = g.title, document.getElementById("btn-delete-event").removeAttribute("hidden")
            },
            ...
            eventDrop: function (t) {
                ...
                Math.floor(11e3 * Math.random()), t.event.title, t.event.start, t.event.allDay, t.event.classNames[0];
                upcomingEvent(y)
            }
        });
    E.render(), i.addEventListener("submit", function (e) {
        e.preventDefault();
        var t, n = document.getElementById("event-title").value,
            ...
            m = !1;
        1 < d.length ? (t = (t = new Date(d[1])).setTime(t.getTime() + 828e5), d = new Date(d[0])) : (e = d, t = document.getElementById("timepicker1").value.trim(), u = document.getElementById("timepicker2").value.trim(), d = new Date(d + "T" + t), o = new Date(e + "T" + u), m = !0);
        var u = y.length + 1;
        !1 === p[0].checkValidity() ? p[0].classList.add("was-validated") : (g ? (g.setProp("id", s), g.setProp("title", n), g.setProp("classNames", [a]), g.setStart(i), g.setEnd(l), g.setAllDay(m), g.setExtendedProp("description", c), g.setExtendedProp("location", r), s = y.findIndex(function (e) {
            return e.id == g.id
        }), y[s] && (y[s].title = n, y[s].start = i, y[s].end = l, y[s].allDay = m, y[s].className = a, y[s].description = c, y[s].location = r), E.render()) : (E.addEvent(r = {
            id: u,
            title: n,
            start: d,
            end: o,
            ...
            description: c,
            location: r
        }), y.push(r)), v.hide(), upcomingEvent(y))
